...
public class Matchmaking {
    ...
    public void startMatchmaking(List<MatchPlayer> players, Match match) {
        ...
        if (players.size() < 4) {
            SendMessage.sendToConsole(
                languageManager.getPluginMessage("matchmaking.not-enough-players"));
            return;
        }
    ...
    private void captainsBalance(List<MatchPlayer> players, Match match) {
        ...
        if (bestCaptains.length >= 2) {
            ...
            SendMessage.broadcast("&4" + captain1Name + " &l&bvs. " + "&9"
                    + captain2Name);
            SendMessage.broadcast("§m---------------------------------");
            match.sendMessage(Component.text(
            languageManager.getConfigurableMessage("picks.choosing")));
            teamBalance(match);
        }
    }
     ...
     */
    private boolean isGoodCaptainPair(PlayerStatsPair captain1, PlayerStatsPair captain2, List<PlayerStatsPair> remainingPlayers) {
        int ratingDifference = Math.abs(captain1.getRating() - captain2.getRating());
        int totalRemainingRating = remainingPlayers.stream().mapToInt(PlayerStatsPair::getRating).sum();
    ...
    /**
     * Encuentra la mejor partición de equipos usando fuerza bruta con variabilidad aleatoria
     * Para más de 14 jugadores, aplica fuerza bruta solo a los 14 mejores y distribuye el resto secuencialmente
     */
    private TeamPartition findBestTeamPartition(List<PlayerStatsPair> players) {
        ...
        // TODO: Asegurar que si hay números impares, el equipo más débil (determinado por team entero) tenga el jugador extra
        String captain1Name = captains.getCaptain1Name();
        String captain2Name = captains.getCaptain2Name();
    ...
    /**
     * Algoritmo híbrido para más de 14 jugadores:
     * - Aplica fuerza bruta a los 14 mejores jugadores
     ...
     */
    private void distributeRemainingPlayers(List<PlayerStatsPair> remainingPlayers, 
                                          List<String> team1Names, List<String> team2Names, 
                                          int totalPlayers) {
    ...
    private void generateCombinationsHelper(List<List<Integer>> combinations, List<Integer> current, int start, int n, int k) {
        if (current.size() == k) {
