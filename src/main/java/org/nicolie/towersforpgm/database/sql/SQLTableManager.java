    ...
    Bukkit.getScheduler().runTaskAsynchronously(TowersForPGM.getInstance(), () -> {
      try (Connection conn = TowersForPGM.getInstance().getDatabaseConnection()) {
        ...
        String createPlayers = "CREATE TABLE IF NOT EXISTS match_players_history ("
            ...
            // Bow stats
            + "longest_bow_kill INT DEFAULT 0,"
            + "bow_damage DOUBLE DEFAULT 0,"
            + "bow_damage_taken DOUBLE DEFAULT 0,"
            + "shots_taken INT DEFAULT 0,"
            + "shots_hit INT DEFAULT 0,"
            // Damage stats
            + "damage_done DOUBLE DEFAULT 0,"
            + "damage_taken DOUBLE DEFAULT 0,"
            // Objective stats
            + "destroyable_pieces_broken INT DEFAULT 0,"
            + "monuments_destroyed INT DEFAULT 0,"
            + "flags_captured INT DEFAULT 0,"
            + "flag_pickups INT DEFAULT 0,"
            + "cores_leaked INT DEFAULT 0,"
            + "wools_captured INT DEFAULT 0,"
            + "wools_touched INT DEFAULT 0,"
            + "longest_flag_hold_millis BIGINT DEFAULT 0,"
            ...
            + "INDEX idx_elo_before (elo_before),"
