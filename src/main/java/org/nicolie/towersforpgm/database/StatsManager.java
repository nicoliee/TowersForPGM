...
public class StatsManager {
  ...
  public static void showStats(
      CommandSender sender, String table, String player, LanguageManager languageManager) {
    boolean isRankedTable = ConfigManager.getRankedTables().contains(table);
    String sql = isRankedTable
        ? "SELECT kills, deaths, assists, damageDone, damageTaken, points, wins, games, winstreak, maxWinstreak, elo, maxElo FROM "
            + table + " WHERE username = ?"
        : "SELECT kills, deaths, assists, damageDone, damageTaken, points, wins, winstreak, maxWinstreak, games FROM "
            + table + " WHERE username = ?";
    Bukkit.getScheduler().runTaskAsynchronously(TowersForPGM.getInstance(), () -> {
      try (Connection conn = TowersForPGM.getInstance().getDatabaseManager().getConnection();
          PreparedStatement stmt = conn.prepareStatement(sql)) {
        ...
        try (ResultSet rs = stmt.executeQuery()) {
          if (rs.next()) {
            ...
            if (isRankedTable) {
              ...
              sender.sendMessage(Queue.RANKED_PREFIX + languageManager.getPluginMessage("stats.elo")
                  + ": " + rank.getPrefixedRank(true)
                  + " " + rank.getColor() + elo
                  + " §7| " + languageManager.getPluginMessage("stats.maxElo") + ": "
                  + rank.getPrefixedRank(true)
                  + " " + maxRank.getColor() + maxElo);
              sender.sendMessage(" ");
            }
            sender.sendMessage(" §7" + languageManager.getPluginMessage("stats.kills") + ": §a"
                + rs.getInt("kills")
                + " §7| " + languageManager.getPluginMessage("stats.deaths") + ": §c"
                + rs.getInt("deaths")
                + " §7| " + languageManager.getPluginMessage("stats.assists") + ": §a"
                + rs.getInt("assists")
                + " §7| " + languageManager.getPluginMessage("stats.damageDone") + ": §a"
                + String.format("%.1f", damageDone)
                + " §7| " + languageManager.getPluginMessage("stats.damageTaken") + ": §c"
                + String.format("%.1f", damageTaken)
                + " §7| " + languageManager.getPluginMessage("stats.points") + ": §a"
                + rs.getInt("points")
                + " §7| " + languageManager.getPluginMessage("stats.wins") + ": §a"
                + rs.getInt("wins")
                + " §7| " + languageManager.getPluginMessage("stats.games") + ": §a" + games
                + " §7| " + languageManager.getPluginMessage("stats.winstreak") + ": §a"
                + rs.getInt("winstreak")
                + " §7| " + languageManager.getPluginMessage("stats.maxWinstreak") + ": §a"
        ...
        SendMessage.sendToDevelopers(
            "§cError al obtener las estadísticas de " + player + " en la tabla " + table);
  ...
  public static void showTop(
    ...
    String sql = "SELECT username, " + category + " FROM " + table + " ORDER BY " + category
    ...
    Bukkit.getScheduler().runTaskAsynchronously(TowersForPGM.getInstance(), () -> {
      try (Connection conn = TowersForPGM.getInstance().getDatabaseManager().getConnection();
        ...
        while (rs.next()) {
          if (category.equalsIgnoreCase("elo") || category.equalsIgnoreCase("maxElo")) {
            ...
            topList
                ...
                .append(rank)
                .append(" ")
                .append(rankObj.getPrefixedRank(true))
                .append(" §a")
                .append(rs.getString("username"))
                .append(" §7- ")
                .append(rankObj.getColor())
                .append(eloValue)
                .append("\n");
          } else {
            topList
        ...
        TowersForPGM.getInstance()
            ...
            .log(
                Level.SEVERE,
                "Error al obtener el top de " + category + " en la tabla " + table,
                e);
        SendMessage.sendToDevelopers("§cError al obtener el top de " + category + " en la tabla "
            + table + " para el jugador " + sender.getName());
        sender.sendMessage("§cHubo un error al obtener los datos.");
      }
    });
  }

  public static void getEloForUsernames(
      String table, List<String> usernames, Consumer<List<PlayerEloChange>> callback) {
  ...
  public static void getEloForUsername(
      String table, String username, Consumer<PlayerEloChange> callback) {
    ...
    Bukkit.getScheduler().runTaskAsynchronously(TowersForPGM.getInstance(), () -> {
      try (Connection conn = TowersForPGM.getInstance().getDatabaseManager().getConnection();
        ...
        TowersForPGM.getInstance()
            .getLogger()
            .log(Level.SEVERE, "Error al obtener el elo del usuario", e);
        callback.accept(null);
      }
    });
  }
