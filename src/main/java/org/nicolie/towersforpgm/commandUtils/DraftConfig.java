...
public class DraftConfig {
  ...
  public void handleDraftSuggestionsCommand(CommandSender sender, Boolean isEnabled) {
    if (isEnabled == null) {
      ...
      String message =
          isDraftSuggestions ? "Draft suggestions are enabled." : "Draft suggestions are disabled.";
  ...
  public void handleSecondGetsExtraPlayerCommand(CommandSender sender, Boolean isEnabled) {
    if (isEnabled == null) {
      ...
      String message = secondGetsExtraPlayer
          ? "Second team gets an extra player."
          : "Second team does not get an extra player.";
      SendMessage.sendToPlayer(sender, message);
      return;
    }
    ConfigManager.setSecondPickBalance(isEnabled);
    String message = isEnabled
        ? "Second team will get an extra player."
        : "Second team will not get an extra player.";
    SendMessage.sendToPlayer(sender, message);
  }
  ...
  public void setPrivateMatch(CommandSender sender, boolean isPrivateMatch) {
    ...
    String message = isPrivateMatch
        ? languageManager.getPluginMessage("privateMatch.true").replace("{map}", mapName)
        : languageManager.getPluginMessage("privateMatch.false").replace("{map}", mapName);
    SendMessage.sendToPlayer(sender, message);
  }
  ...
  public void setDraftOrder(CommandSender sender, String order) {
    if (order == null || order.isEmpty()) {
      ...
      String message =
          languageManager.getPluginMessage("draft.currentOrder").replace("{order}", currentOrder);
      SendMessage.sendToPlayer(sender, message);
      return;
    }
    ...
    String success = languageManager.getPluginMessage("draft.orderSet").replace("{order}", order);
  ...
  public void setMinDraftOrder(CommandSender sender, int size) {
    if (size < 1) {
      ...
      String message = languageManager
          .getPluginMessage("draft.currentSize")
    ...
    String message =
        languageManager.getPluginMessage("draft.sizeSet").replace("{size}", String.valueOf(size));
    SendMessage.sendToPlayer(sender, message);
  }
}
