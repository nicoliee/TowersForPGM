...
public class RankedConfig {
    ...
    public void size(CommandSender sender, String size) {
        ...
        try {
            ...
            SendMessage.sendToPlayer(sender, "Invalid size format. Please enter a number between 2 and 12.");
        }
    }
    ...
    public void deleteTable(CommandSender sender, String table) {
        if (!ConfigManager.getRankedTables().contains(table)) {
            SendMessage.sendToPlayer(sender, languageManager.getPluginMessage("table.notFound").replace("{table}", table));
            return;
        }
    ...
    public void addMap(CommandSender sender){
        ...
        if (ConfigManager.getRankedMaps().contains(map)) {
            SendMessage.sendToPlayer(sender, languageManager.getPluginMessage("map.exists").replace("{map}", map));
            return;
        }
    ...
    public void removeMap(CommandSender sender) {
        ...
        String map = match.getMap().getName();
        if (!ConfigManager.getRankedMaps().contains(map)) {
            SendMessage.sendToPlayer(sender, languageManager.getPluginMessage("map.notFound").replace("{map}", map));
            return;
        }
    ...
    public void matchmaking(CommandSender sender, Boolean isEnabled) {
        if (isEnabled == null) {
            ...
            String message = isRankedMatchmaking ? "Ranked matchmaking is enabled." : "Ranked matchmaking is disabled.";
            SendMessage.sendToPlayer(sender, message);
            return;
        }
