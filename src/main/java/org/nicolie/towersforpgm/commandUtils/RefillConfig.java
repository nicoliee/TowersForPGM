...
public class RefillConfig {
    ...
    public void addRefillLocation(CommandSender sender, String mapName, Location loc) {
        ...
        if (!(block.getState() instanceof Chest)) {
            SendMessage.sendToPlayer(sender, languageManager.getPluginMessage("refill.chestNotFound")
        ...
        for (int i = 1; refillConfig.contains("refill." + mapName + ".c" + i); i++) {
            ...
            if (storedCoords.equals(coords)) {
                SendMessage.sendToPlayer(sender, languageManager.getPluginMessage("refill.chestAlreadyExists")
                        .replace("{coords}", coords));
                return;
            }
        }
        ...
        SendMessage.sendToPlayer(sender, languageManager.getPluginMessage("refill.chestSet")
                    .replace("{coords}", coords));
    ...
    public void removeRefillLocation(CommandSender sender, String mapName, Location loc) {
        ...
        for (int i = 1; refillConfig.contains("refill." + mapName + ".c" + i); i++) {
            ...
            if (storedCoords.equals(coords)) {
                ...
                plugin.saveRefillConfig();
                plugin.reloadRefillConfig();
                SendMessage.sendToPlayer(sender, languageManager.getPluginMessage("refill.chestDeleted")
                        .replace("{coords}", coords));
                found = true;
                break;
            }
