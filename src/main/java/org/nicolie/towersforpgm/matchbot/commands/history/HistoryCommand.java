...
public class HistoryCommand extends ListenerAdapter {
  ...
  public static void register() {
    ...
    if (jda != null) {
      ...
      var command = jda.upsertCommand(
              NAME, LanguageManager.message("matchbot.cmd.history.description"))
          .addOption(
              net.dv8tion.jda.api.interactions.commands.OptionType.STRING,
              LanguageManager.message("matchbot.cmd.history.matchid"),
              LanguageManager.message("matchbot.cmd.history.desc-matchid"),
              true,
              true);
  ...
  public void onSlashCommandInteraction(SlashCommandInteractionEvent event) {
    ...
    OptionMapping matchIdOpt =
        event.getOption(LanguageManager.message("matchbot.cmd.history.matchid"));
      ...
      MatchHistoryManager.getMatch(matchId).whenComplete((history, throwable) -> {
        if (throwable != null) {
          hook.editOriginalEmbeds(HistoryEmbed.createError(
                      matchId, LanguageManager.message("matchbot.cmd.history.error"))
                  .build())
              .queue();
          return;
        }
        if (history == null) {
          hook.editOriginalEmbeds(HistoryEmbed.createError(
                      matchId, LanguageManager.message("matchbot.cmd.history.notfound"))
                  .build())
              .queue();
          return;
        }
