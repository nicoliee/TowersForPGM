...
public class TopCommand extends ListenerAdapter {
  ...
  private static final String DESC =
      lang.getPluginMessage("matchbot.top.description");
  ...
  private static final String OPT_PER_GAME = lang.getPluginMessage("matchbot.top.perGame");
  private static final String DESC_PER_GAME = lang.getPluginMessage("matchbot.top.desc-perGame");
  ...
  public void onSlashCommandInteraction(SlashCommandInteractionEvent event) {
    ...
    if (perGame && !stat.isStatPerGame()) {
      event
          .reply(lang.getPluginMessage("matchbot.top.perGame-restriction").replace("{stat}", stat.getDisplayName()))
          .setEphemeral(true)
          .queue();
      return;
    }
        ...
        .queue(hook -> CompletableFuture.runAsync(() -> {
          try {
            ...
            if (result.getData().isEmpty()) {
              hook.sendMessage(lang.getPluginMessage("matchbot.top.invalid-column").replace("{stat}", stat.getDisplayName()).replace("{table}", table))
                  .setEphemeral(true)
                  .queue();
              return;
            }
            ...
            String token =
                org.nicolie.towersforpgm.matchbot.commands.top.TopPaginationListener
                    .createStateToken(
                        table,
                        stat,
                        perGame,
            ...
            hook.sendMessage(lang.getPluginMessage("matchbot.top.error").replace("{error}", e.getMessage()))
                .queue();
          }
        }));
  }
