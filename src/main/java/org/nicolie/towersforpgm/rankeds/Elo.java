...
/**
 * Clase encargada de gestionar el sistema de ranking Elo para partidas competitivas.
 * El sistema Elo es un método de calificación utilizado para calcular los niveles 
 * relativos de habilidad de los jugadores en juegos de suma cero.
 * 
 * Esta implementación incluye factores adicionales como:
 * - Habilidad individual del jugador comparada con su equipo
 * - Diferencia de fuerza entre equipos
 * - Límites mínimos y máximos de cambio de Elo
 */
public class Elo {
    private static final int K_FACTOR = 32;

    /**
     * Método principal para calcular y aplicar cambios de Elo después de una partida.
     * Procesa tanto a los ganadores como a los perdedores, calculando cambios individualizados
     * basados en múltiples factores de rendimiento.
     * 
    ...
    public static CompletableFuture<List<PlayerEloChange>> addWin(List<MatchPlayer> winners, List<MatchPlayer> losers) {
        ...
        StatsManager.getEloForUsernames(table, usernames, eloList -> {
            ...
            winners.forEach(winner -> {
                ...
                int individualEloChange = calculateIndividualEloChange(currentElo, winnersAvgElo, losersAvgElo, true);
                
                int newElo = currentElo + individualEloChange;
                int maxElo = Math.max(newElo, playerElo.getMaxElo()); // Actualizar Elo máximo si es necesario
            ...
            losers.forEach(loser -> {
                ...
                // Calcular el cambio individual de Elo basado en el rendimiento del jugador vs expectativa
                int individualEloChange = calculateIndividualEloChange(currentElo, losersAvgElo, winnersAvgElo, false);
    ...
    /**
     * Obtiene y ordena los jugadores por su Elo actual de mayor a menor.
     * También calcula el Elo promedio del grupo de jugadores.
     * 
    ...
    /**
     * Calcula el cambio de Elo individual para un jugador basado en múltiples factores.
     * Este algoritmo considera:
     * 1. La puntuación esperada según la fórmula estándar de Elo
     * 2. La habilidad individual del jugador comparada con su equipo
     * 3. La diferencia de fuerza entre los equipos
     * 
     ...
     */
    private static int calculateIndividualEloChange(int playerElo, double teamAvgElo, double opponentAvgElo, boolean isWinner) {
