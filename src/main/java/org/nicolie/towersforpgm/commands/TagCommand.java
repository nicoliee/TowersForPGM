...
public class TagCommand implements CommandExecutor{
    ...
    public boolean onCommand(CommandSender sender, Command command, String label, String[] args) {
        if (!TowersForPGM.getInstance().isMatchBotEnabled()){return true;}
        ...
        if (!ConfigManager.getRankedMaps().contains(map)) {
            player.sendWarning(Component.text(Queue.RANKED_PREFIX + languageManager.getPluginMessage("ranked.notRankedMap")));
            return true;
        }
        if (player.isParticipating()) {
            player.sendWarning(Component.text(languageManager.getPluginMessage("ranked.matchbot.tagNotAvailable")));
            return true;
        }
        ...
        if (now - lastExecution < COOLDOWN_MILLIS) {
            ...
            player.sendWarning(Component.text(languageManager.getPluginMessage("ranked.matchbot.tagCooldown")
                    .replace("{time}", time)));
            return true;
        }
        lastExecution = now;
    ...
    private void sendEmbed(CommandSender sender, Match match, MatchPlayer player){
        ...
        StatsManager.getEloForUsernames(ConfigManager.getRankedDefaultTable(), usernames, eloChanges -> {
            EmbedBuilder embed = Embed.notifyPlayers(sender, match, eloChanges);
                MatchBot.getInstance().getBot().setEmbedThumbnail(match.getMap(), embed, MatchBot.getInstance().getBot());
                MatchBot.getInstance().getBot().sendMatchEmbed(embed, match, ConfigManager.getRankedChannel(), ConfigManager.getRankedRoleID());
            match.sendMessage(Component.text(Queue.RANKED_PREFIX + languageManager.getPluginMessage("ranked.matchbot.tagSent")
