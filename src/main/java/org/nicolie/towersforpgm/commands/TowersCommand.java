...
public class TowersCommand implements CommandExecutor, TabCompleter {
    ...
    public boolean onCommand(CommandSender sender, Command command, String label, String[] args) {
        // Validación básica: asegurarse de que haya al menos un argumento
        if (args.length == 0) {
            sender.sendMessage("§c/towers <draft|preparation|stats|help>");
            return true;
        }
        String mainArg = args[0].toLowerCase(); // Primer argumento (comando principal)

        switch (mainArg) {
            case "draft":
                // Manejo de /towers draft ...
                if (args.length == 1) {
                    sender.sendMessage("§c/towers draft <suggestions|timer|private>");
                    return true;
                }
            ...
            case "stats":
                if (args.length == 1) {
                    sender.sendMessage("§c/towers stats <toggle|default|add|remove|list|addMap|removeMap>");
                    return true;
                }
                ...
                if (args.length == 2) {
                    switch (args[1].toLowerCase()) {
                        ...
                        case "stats":
                            ...
                            break;
                        default:
                            sender.sendMessage("§cSección de ayuda desconocida.");
                            break;
                    }
                } else {
    ...
    public List<String> onTabComplete(CommandSender sender, Command command, String alias, String[] args) {
        if (args.length == 1) {
            List<String> base = Arrays.asList("draft", "preparation", "stats", "help");
        ...
        switch (first) {
            case "draft":
                if (args.length == 2) {
                    List<String> options = Arrays.asList("suggestions", "timer", "private");
            ...
            case "stats":
                if (args.length == 2) {
                    List<String> options = Arrays.asList("toggle", "default", "add", "remove", "list", "addMap", "removeMap");
                    return filterPrefix(options, args[1]);
                }
                if (args.length == 3) {
                    String sub = args[1].toLowerCase();
            ...
            case "help":
                if (args.length == 2) {
                    List<String> options = Arrays.asList("draft", "preparation", "stats");
                    return filterPrefix(options, args[1]);
                }
                break;
        }
    ...
    // Método utilitario para filtrar según lo escrito
    ...
    // Método utilitario para sugerencias numéricas básicas
    private List<String> suggestNumbers(int argPosition) {
        // Suponiendo que se permiten hasta 3 argumentos para coords y 1 para tiempo
