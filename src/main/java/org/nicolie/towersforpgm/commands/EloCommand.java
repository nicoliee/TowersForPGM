    ...
    StatsManager.getEloForUsername(table, player, eloChange -> {
      if (eloChange == null) {
        ...
        sender.sendMessage(Queue.RANKED_PREFIX + Rank.getRankByElo(elo).getPrefixedRank(true)
            + Rank.getRankByElo(elo).getColor() + " " + player + " §r- Elo: "
            + Rank.getRankByElo(elo).getColor() + elo + " §f| Max Elo: "
            + Rank.getRankByElo(maxElo).getPrefixedRank(true) + " "
            + Rank.getRankByElo(maxElo).getColor() + maxElo);
      }
    });
    return true;
  }
